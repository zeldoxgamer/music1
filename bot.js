...
client.on('message', async msg => { 
	...
	if (command === `+1play`) {
        ...
        if (!permissions.has('CONNECT')) {
			...
			return msg.channel.send("ليست عندي برمشن لدخول الرووم");
        }
        
		if (!permissions.has('SPEAK')) {

			return msg.channel.send("ليست عندي البرمشن للتكلم بهدا الروم");
		}

		if (!permissions.has('EMBED_LINKS')) {

			return msg.channel.sendMessage("ليست عندي البرمشن لتشغيل الاغنية")
		}
...
/////////////////					
					try {
						...
						console.error(err);
						return msg.channel.send('لم يتم اختيار الرقم');
                    }
					...
					console.error(err);
					return msg.channel.send("لم اجد هده الاغنية");
				}
			}
...
**Now playing :** **${serverQueue.songs[0].title}**`)
		...
		return msg.channel.send('ليس هناك لائحة');
	}
...
client.on('message', message => {
    if (message.content === '+1help') {
	    ...
	    .addField('+1sw', 'لتغيير حالة البوت ')
    	 .addField('+1np', 'اظهار الاغنية اللي انت مشغلها حاليا')
